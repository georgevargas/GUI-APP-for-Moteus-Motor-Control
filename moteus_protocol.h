...
#error "only little endian architectures supported"
  ...
  void WriteVelocityLimit(double value, Resolution res) {
    WriteMapped(value, 0.1, 0.00025, 0.00001, res);
  }
...
struct PositionResolution {
  ...
  Resolution accel_limit = Resolution::kFloat;
};
...
inline void EmitPositionCommand(WriteCanFrame *frame,
  ...
  if (combiner.MaybeWrite()) {
    frame->WriteVelocityLimit(command.velocity_limit, resolution.velocity_limit);
  }
  if (combiner.MaybeWrite()) {
    frame->WriteVelocityLimit(command.accel_limit, resolution.accel_limit);
  }
}
...
inline void EmitSetOutputNearest(WriteCanFrame *frame, double value) {
  frame->Write<int8_t>(Multiplex::kWriteFloat | 0x01);     // 0d - WRITE_REGISTERS - Type.F32 1 register
  frame->Write<int8_t>((Register::kRezero & 0x7f) | 0x80); // send out 2 byte register number because its value is greater than 0x7f, lsb first.
  frame->Write<int8_t>(Register::kRezero>>7);              // b002 - Starting at reg 0x130(REZERO)
  frame->Write<float>(value);
}
...
struct QueryCommand {
  ...
  Resolution TrajectoryComplete = Resolution::kInt16;
  ...
  bool any_set() const {
    return mode != Resolution::kIgnore || position != Resolution::kIgnore ||
           ...
           TrajectoryComplete != Resolution::kIgnore;
  }
};
...
inline void EmitQueryCommand(WriteCanFrame *frame,
  ...
  {
        WriteCombiner<1> combiner(frame, 0x10, Register::kTrajectoryComplete,
                              {
                                      command.TrajectoryComplete
                              });
  ...
  while (true) {
    ...
    switch (static_cast<Register>(std::get<1>(entry))) {
      ...
      case Register::kTrajectoryComplete: {
          result.TrajectoryComplete = parser.ReadInt(res) != 0;
          break;
      }
